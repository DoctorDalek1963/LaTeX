...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\node[label={[label distance=0.25]90:$w_1$}] (w1) at (circle center) {};
		...
		\node[label={[label distance=0.25]270:$w_2$}] (w2) at (other point) {};
	...
	\begin{minipage}{0.33\linewidth}
		\setlength{\parskip}{1em}
		For a complex number $w_1 = x + yi$, the locus of points $|z - w_1| = r$ \textcolor{gray}{$\Leftrightarrow \left|z - (x + yi)\right| = r$} is a circle of radius $r$ around the point $w_1$.

		$|z - w_1| = |z - w_2|$ is the perpendicular bisector of the line joining $w_1$ and $w_2$.

		$\arg(z - w_2) = \theta$ is a half-line from, but not including, the point $w_2$ making an angle $\theta$ from the real axis.
	\end{minipage}
\end{figure}
